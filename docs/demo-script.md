...
### mock-rule-demo：稳定演示 PASS / REVIEW / REJECT
...
该模式依赖 mock fields，适合稳定演示业务规则三分支。

### real-ocr-demo：真实 OCR 链路演示

真实 OCR 演示使用任务 fixture：
...
```text
.local/leave_audit_fixtures/sick-diagnosis-proof.jpg
```
...
详见：[`docs/real-ocr-demo.md`](real-ocr-demo.md)。

真实 OCR 输出受图片质量、字段识别和规则匹配影响，不保证稳定产出 PASS / REVIEW / REJECT 三分支；这属于预期。需要稳定三分支演示时使用 mock-rule-demo。
